...
pub fn enum_iter<T: Enum<()> + Copy, R: RangeBounds<T>>(r: R) -> EnumIter<T> {
    ...
    if i > end {
        panic!(
            "slice index starts at ordinal {} but ends at ordinal {}",
            i, end
        );
    } else if end > T::POSSIBLE_VALUES {
        panic!(
            "ordinal {} out of range for enum of length {}",
            i,
            T::POSSIBLE_VALUES
        );
    }
...
impl EntityKind {
    pub fn dir(self) -> &'static str {
        static DIRS: &[&str] = &[
            "items", "critters", "scenery", "walls", "tiles", "misc", "intrface", "inven", "heads",
            "backgrnd", "skilldex",
        ];
        DIRS[self as usize]
    }
...
pub fn read_gam(rd: &mut impl BufRead, tag: &str) -> io::Result<Vec<i32>> {
    ...
    for l in lines {
        ...
        let v = l[i + 2..].trim();
        let v = btoi::btoi::<i32>(v.as_bytes()).map_err(|_| {
            Error::new(
...
GVAR_2                    :=   -123;    //      (2) blah blah blah";
        assert_eq!(
            read_game_global_vars(&mut BufReader::new(Cursor::new(s))).unwrap(),
            [0, 100, -123]
        );
    }
...
MVAR_2:=-1;    //      (2)";
        assert_eq!(
            read_map_global_vars(&mut BufReader::new(Cursor::new(s))).unwrap(),
            [123, 0, -1]
        );
    }
...
key2
     ...
     key=
        ...
        let exp = &[
            ("sec tion_1", &[("key", "va \t lue"), ("key2", "")]),
            ("section2", &[("key", ""), ("key2", "stuff")]),
        ];
